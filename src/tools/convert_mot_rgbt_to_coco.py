...
DATA_PATH = "/media/jianbo/ioe/motdata/UniRTL_MOT/MOT"
...
if __name__ == '__main__':
    ...
    for split in SPLITS:
        ...
        out = {'images': [], 'annotations': [], 'videos': [],
               'categories': [
                   {'id': 1, 'name': 'people'}]
               }  # {'id': 1, 'name': 'pedestrian'},{'id': 2, 'name': 'car'} for VTMOT dataset
        ...
        for seq in sorted(seqs):
            ...
            vi_img_path = os.path.join(seq_path, 'rgb')
            ...
            for i in range(len(vi_images)):
                ...
                image_info = {'vi_file_name': '{}/rgb/{}'.format(seq, vi_images[i]),  # image name.
                              'ir_file_name': '{}/IR/{}'.format(seq, ir_images[i]),
                              'id': image_cnt + i + 1,  # image number in the entire training set.
                              'frame_id': i + 1,  # image number in the video sequence, starting from 1.
                              'prev_image_id': image_cnt + i if i > 0 else -1,  # 前一帧图片序号
                              'next_image_id': image_cnt + i + 2 if i < num_images - 1 else -1,  # 后一帧图片序号
                              'video_id': video_cnt,
                              'height': height,
                              'width': width}
            ...
            if split != 'test':
                ...
                print('{}: {} ann images'.format(seq, int(anns[:, 0].max()) - int(anns[:, 0].min()) + 1))
