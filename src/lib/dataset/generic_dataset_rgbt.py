...
class GenericDataset(data.Dataset):
  ...
  def mask_area_random(self, vi_img, pre_vi_img, ir_img, pre_ir_img, H, W, mask_ratio=0.2, patch_length=16):

    patch_h = int(H / patch_length)
    patch_w = int(W / patch_length)

    patch_area = patch_h * patch_w  #
    len_keep = int(patch_area * mask_ratio)

    noise = np.random.rand(patch_area)

    ids_shuffle = np.argsort(noise)  # 从小到大排序后的索引

    rgb_nokeep = ids_shuffle[0:len_keep]
    t_nokeep = ids_shuffle[patch_area - len_keep:patch_area]

    mask_rgb = np.ones([patch_h * patch_w])
    mask_rgb[rgb_nokeep] = 0

    mask_t = np.ones([patch_h * patch_w])
    mask_t[t_nokeep] = 0

    mask_rgb = mask_rgb.reshape([patch_h, patch_w])
    mask_t = mask_t.reshape([patch_h, patch_w])

    mask_rgb = np.repeat(mask_rgb, patch_length, axis=1)
    mask_rgbb = np.repeat(np.repeat(mask_rgb, patch_length, axis=0)[:, :, np.newaxis], 3, axis=2)

    mask_t = np.repeat(mask_t, patch_length, axis=1)
    mask_tt = np.repeat(np.repeat(mask_t, patch_length, axis=0)[:, :, np.newaxis], 3, axis=2)

    if mask_tt.shape[0] != H or mask_tt.shape[1] != W:
      pad_height = H % patch_length
      pad_width = W % patch_length
      mask_rgbb = np.pad(mask_rgbb, pad_width=((pad_height // 2, pad_height - pad_height // 2),
                                               (pad_width // 2, pad_width - pad_width // 2), (0, 0)),
                         mode='constant', constant_values=1)
      mask_tt = np.pad(mask_tt, pad_width=((pad_height // 2, pad_height - pad_height // 2),
                                           (pad_width // 2, pad_width - pad_width // 2), (0, 0)),
                       mode='constant', constant_values=1)

    out_vi_img = np.where(mask_rgbb == 0, 0, vi_img)
    out_ir_img = np.where(mask_tt == 0, 0, ir_img)

    out_pre_vi_img = np.where(mask_rgbb == 0, 0, pre_vi_img)
    out_pre_ir_img = np.where(mask_tt == 0, 0, pre_ir_img)

    return out_vi_img, out_pre_vi_img, out_ir_img, out_pre_ir_img
  ...
  def __getitem__(self, index):
    ...
    # vi_img, pre_vi_image, ir_img, pre_ir_image = self.mask_area_random(vi_img, pre_vi_image, ir_img, pre_ir_image, H=height, W=width, mask_ratio=0.2, patch_length=16)
